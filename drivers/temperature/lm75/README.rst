...
the actual milli-Celcius temperature reading. We use the function 
``lm75_raw_to_millicelcius`` to convert the raw temperature value to 
milli-Celcius. Conversely, if we have a milli-Celcius and want to have a raw 
temperature for writing to the over-temperature and hysteresis, we can use the
``lm75_millicelcius_to_raw`` function.
...
.. code-block:: C
   ...
   int iio_example_main()
   {
           ...
           ret = lm75_iio_init(&iio_lm75, lm75);
           if (ret)
