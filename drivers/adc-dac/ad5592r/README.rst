...
ontop of SPI and I2C interface respectively. Each of the 8 I/Ox pins can be 
...
When an I/Ox pin is configred as an analog output, it is driven by a 12-bit DAC.
...
the ``[chip]_init()`` pattern where ``ad5592r_init()`` is for initializaing the
ad5592r device and ``ad55933_init()`` is for initializing the ad5593r device.
...
from either ad55924 or ad5593r pins and then use ``ad5592r_gpio_set()`` to 
write to the same.
...
.. code-block:: C
        ...
        ret = ad5593r_write_dac(ad5592r, 0, 1024);
        if (err)
                return ret;

        /* read from AD5593R ADC channel 1 */
        ret = ad5593r_read_adc(ad5592r, 1, &adc_val);
        if (err)
                return ret;

        /* Cleanup resources */
        ad5593r_remove(ad5593r);
        add592r_remove(ad5592r);
